  ...
  <Container>
    <div class="dataClean">
      <el-card shadow="never" style="height: 100%; overflow: auto">
        ...
        <el-divider />
        <!-- 一键审查 -->
        <div v-if="radio == '一键审查'" class="keyReview">
          <!-- 一键审查规则 -->
          <div class="formRule" style="width: 1000px">
            <el-form
              ref="formLabelAlign"
            ...
            >
              <el-form-item label="一键审查规则:" style="position: relative">
                <el-select
                  v-model="value1"
                  multiple
                  collapse-tags
              ...
              <!-- v-model="currentRow.sql"enterTextarea -->
              <!-- <el-input
                  v-model="enterTextarea"
                  type="textarea"
              ...
              >
                ...
                <!-- <el-button
                  ...
                  @click="xinzen"
                >保存</el-button> -->
                <!-- <el-button plain size="medium" :disabled="enterTextarea?false:true" @click="execute">执行</el-button> -->
                <el-button
                  plain
          ...
          <el-divider />
          <template>
            <el-tabs v-if="value1.length>0" v-model="activeNameRule" :table-list="tabsLableRule" type="card" @tab-click="handleClick">
              <el-tab-pane v-for="(item) in tabsLableRule" :key="item.id" :label="item.name" :name="item.id" />
            </el-tabs>
          </template>
          <!-- 表格内容 -->
          <div v-if="value1.length>0 && tableData.length>0" class="table-bon">
...
export default {
  ...
  methods: {
    ...
    // 一键审查规则下拉选择sql语句
    selOption(value) {
      console.log(value)  // ["1c4bb838ce5d45eebbd34001146d3cf6","1c4bb838ce5d45eebbd34001146d3cf6" __ob__: Observer]
      this.tabsLableRule = []
      for (const i in value) {
        const sqlItem = this.options.find(item => item.id === value[i])
    ...
    // 执行
    // execute() {
    //   if (this.enterTextarea === '') {
    //     // const props = {
    //     //   pageNum: this.pager.pageNum,
    //     //   pageSize: this.pager.pageSize,
    //     //   id: this.tabsLableRule.toString()
    //     // }
    //     // API.getExecuteRule(props).then(res => {
    ...
    handleSizeChangeReview(val) {
      this.pager.pageSize = val
      // this.selOption(this.activeNameRule)
      const props = {
        id: this.activeNameRule,
        pageNum: this.pager.pageNum,
        pageSize: this.pager.pageSize
