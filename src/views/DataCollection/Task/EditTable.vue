  ...
  <div class="total-section">
    <div class="toolbar-section">
      ...
      <div class="button-section">
        <el-button size="mini" icon="el-icon-check" class="el-button el-button--primary is-plain":disabled="operation==='detail'" @click="onHandleSave">保存</el-button>
        <el-button size="mini" icon="el-icon-plus" class="el-button el-button--success is-plain" :disabled="operation==='detail'" @click="onHandleAdd">添加字段</el-button>
        <el-button size="mini" icon="el-icon-minus" class="el-button el-button--danger is-plain" :disabled="operation==='detail'" @click="onHandleDelete">删除字段</el-button>
      </div>
    </div>
    ...
    <el-dialog
      ...
      width="30%"
    >
      <el-input v-model="categoryName" @blur="blurs" />
...
export default {
  ...
  watch: {
    form: {
      handler(val) {
        this.labelName = val.categoryName
      },
      deep: true
    }
  },
  methods: {
    blurs() {
      if (this.categoryName.length > 32) {
        this.$message({
          message: '数据类型长度不能超过32个字符',
          type: 'warning'
        })
        return
      }
    ...
    submitTreeData() {
      if (this.opration === 'appendTree') {
        if (this.categoryName && this.categoryName.length < 32) {
    ...
    onHandleDelete() {
      ...
      this.$confirm('是否删除, 是否继续?', '提示', {
        ...
        params.tableColumnData = deleteList
        API.customizeDelete(params).then(res => {
          const { code, msg } = res
          if (code === 200) {
